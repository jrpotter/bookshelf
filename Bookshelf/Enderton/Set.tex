...
\subsection{\verified{Proper Subset Size}}%
\hyperlabel{sub:proper-subset-size}
  ...
  \code{Bookshelf/Enderton/Set/Chapter\_6}
    {Enderton.Set.Chapter\_6.proper\_subset\_size}
...
\hyperlabel{sub:exercise-6.7}
  ...
  \begin{proof}
    ...
    \paragraph{($\Leftarrow$)}%
      ...
      Let $B = \img{f^{-1}}{\{y\}}$.
      Then $x_1, x_2 \in B$.
      Since $B \subseteq A$, \nameref{sub:proper-subset-size} indicates that
        there exist natural numbers $m_1, n_1 \in \omega$ such that
        $B \equin m_1$, $A \equin n_1$, and $m_1 \leq n_1$.
      Define $f' \colon (A - B) \rightarrow (A - \{y\})$ as the
        \nameref{ref:restriction} of $f$ to $A - B$.
      Since $\ran{f} = A$, it follows $\ran{f'} = A - \{y\}$.
      By \nameref{sub:finite-domain-range-size}, there exist natural numbers
        $m_2, n_2 \in \omega$ such that $\dom{f'} \equin m_2$,
        $\ran{f'} \equin n_2$, and $m_2 \geq n_2$.
      Thus
        \begin{align*}
