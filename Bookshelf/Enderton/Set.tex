...
\subsection{\pending{Proper Subset Size}}%
\hyperlabel{sub:proper-subset-size}

  \begin{lemma}
    Let $A$ be a finite set and $B \subset A$.
    Then there exist natural numbers $m, n \in \omega$ such that
      $B \equin m$, $A \equin n$, and $m < n$.
  \end{lemma}

  \begin{proof}

    Let $A$ be a \nameref{ref:finite-set} and $B$ be a
      \nameref{ref:proper-subset} of $A$.
    ...
    By \nameref{sub:trichotomy-law-natural-numbers}, there are two mutually
      exclusive possibilities to consider:

    \paragraph{Case 1}%

      Assume $m = n$.
      Then by \nameref{sub:theorem-6a}, $A \equin n \equin m \equin B$.
      But by \nameref{sub:corollary-6c}, no finite set is equinumerous to a
        proper subset of itself.
      This is a contradiction.
    ...
    \paragraph{Conclusion}%
      ...
      The \nameref{sub:trichotomy-law-natural-numbers} tells us that $m < n$ is
        the only possibility.
      Hence there indeed exists natural numbers $m, n \in \omega$ such that
        $B \equin m$, $A \equin n$, and $m < n$.

  \end{proof}

\subsection{\sorry{Finite Domain and Range Size}}%
...
\hyperlabel{sub:exercise-6.7}
  ...
  \begin{proof}
    ...
    \paragraph{($\Leftarrow$)}%
      ...
      TODO: Two cases, one when $A = \emptyset$.

      Suppose $\ran{f} = A$.
      Let $x_1, x_2 \in A$ such that $f(x_1) = f(x_2) = y$.
      Define $B = \{ x \in A \mid f(x) = y \}$ which we note contains at least
        members $x_1$ and $x_2$.
      Because $B$ is a proper subset of finite set $A$, \nameref{} indicates
        there exist $m, n \in \omega$ with $m < n$ such that $B \equin m$ and
        $A \equin n$.
      Define $f' \colon (A - B) \rightarrow (A - \{y\})$ as the
        \nameref{ref:restriction} of $f$ by $A - B$.

      By construction, $\dom{f'} \equin n - m$ and $\ran{f'} \equin n - 1$.
      Since $\ran{f'} = A - \{y\}$, for all $d \in \ran{f'}$, there exists some $c \in A - B$ such that $f'(c) = d$.
      Since functions are single-valued by definition, this implies $n - m \geq n - 1$.
      But $n - 1$ cannot be less than $n - m$ because $m \geq 1$.
      Thus it follows $n - m = n - 1$, i.e. $m = 1$.
      Hence $x_1 = x_2$.
